...
namespace common
{
    public class Database : IDisposable
    {
        ...
        public Task UpdateOnrane(DbAccount acc, int amount, ITransaction transaction = null)
        {
            ...
            if (amount > 0)
                trans.HashIncrementAsync(acc.Key, "totalOnrane", amount)
                    .ContinueWith(t =>
                    {
                        if (!t.IsCanceled)
                            acc.TotalOnrane = (int)t.Result;
                    });

            var task = trans.HashIncrementAsync(acc.Key, "onrane", amount)
                .ContinueWith(t =>
                {
                    if (!t.IsCanceled)
                        acc.Onrane = (int)t.Result;
                });
        ...
        public Task UpdateKantos(DbAccount acc, int amount, ITransaction transaction = null)
        {
            ...
            if (amount > 0)
                trans.HashIncrementAsync(acc.Key, "totalKantos", amount)
                    .ContinueWith(t =>
                    {
                        if (!t.IsCanceled)
                            acc.TotalKantos = (int)t.Result;
                    });

            var task = trans.HashIncrementAsync(acc.Key, "kantos", amount)
                .ContinueWith(t =>
                {
                    if (!t.IsCanceled)
                        acc.TotalKantos = (int)t.Result;
